...
def maybe_get_vec(word, vecs, mwp ="split"):
    ...
    embeds = encoder(preprocessor(constant))["default"]
    embeds = normalization(embeds)
    # output = open("crazy_test_output", "w")
    # # output.write(embeds.type)
    # output.write(str(embeds.numpy()))
    # output.close()
    #print(embeds)
    return embeds.numpy()
    # # encoded_input = tokenizer(word, return_tensors='pt')
    # # index = encoded_input.index(word)
    # marked_text = "[CLS] " + word + " [SEP]"
    # tokenized_text = tokenizer.tokenize(marked_text)
    # index = tokenized_text.index(word)
    # # tokenized_text = tokenizer.tokenize(encoded_input)
    ...
    # ret_var = token_vecs_cat[index]
    # ret_var_reshaped = ret_var.reshape(1, -1)
    # #ret_var = np.copy(token_vecs_cat)
    # #np.savetxt("out_file.txt", token_vecs_cat.numpy(), delimiter='\t')
    # print(ret_var_reshaped.numpy())
    # return ret_var_reshaped.numpy()
    # model.eval()
    # output = model(**encoded_input)
    # sent_embedding_reshaped = output.reshape(1, -1)
    # output.eval()
    # v = output.numpy()
...
def maybe_sim(a, b, vecs, cutoff=0.5, diffsense=0.5, simfun=cosine_sim, mwp ="split"):
    ...
    #similarity 
    # SHIRA chamged here
    sim = np.matmul(a_vec, np.transpose(b_vec))
