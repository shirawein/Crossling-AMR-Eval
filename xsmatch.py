...
# def get_words(instance, attributes):
#     words = []

#     for instance_label, prefix, word in instance:
#         words.append(word)

#     for tag, prefix, word in attributes:
#         words.append(word)

#     return words


# def translate_word(french_words, eng_words):

# 	# print("FRENCH WORDSSSS")
# 	# print(french_words)
# 	# print("ENGLISH WORDSSSS")
# 	# print(eng_words)

# 	align_dict = {}

# 	# f = open("example_demo.txt", "w")

# 	french_english_location = working_directory + "/french_english_text"

# 	french_english_text = open(french_english_location, "w")
...
# 		curr_str = str(item)
# 		if item in english_words_temp:
# 			curr_str = item + "_two"
# 			if curr_str in english_words_temp:
# 				curr_str = item + "_three"
# 				if curr_str in english_words_temp:
# 					curr_str = item + "_four"
# 		french_english_text.write(curr_str)
# 		english_words_temp.append(curr_str)
# 		french_english_text.write(" ")

# 	french_english_text.write(" ||| ")

# 	for item in french_words:
# 		curr_str = str(item)
# 		if item in french_words_temp:
# 			curr_str = item + "_two"
# 			if curr_str in french_words_temp:
# 				curr_str = item + "_three"
# 				if curr_str in french_words_temp:
# 					curr_str = item + "_four"
# 		french_english_text.write(curr_str)
# 		french_words_temp.append(curr_str)
# 		french_english_text.write(" ")

# 	french_english_text.close()

# 	# forward_location = working_directory + "/forward_align"
# 	forward_align = open("forward_align", "w")

# 	p = subprocess.Popen(["./fast_align", "-i", "french_english_text", "-d", "-o", "-v"], cwd=working_directory, stdout = forward_align)
...
# 	with open("forward_align") as f:
# 		lines = f.readlines()
# 		alignment = lines[0].rstrip("\n")

# 	split_alignment = alignment.split(" ")

# 	for item in split_alignment:
# 		split_item = item.split("-")
# 		eng_index = split_item[0]
# 		curr_eng_word = eng_words[int(eng_index)]
# 		french_index = split_item[1]
# 		curr_french_word = french_words[int(french_index)]
# 		align_dict[curr_french_word] = curr_eng_word

# 	# lines = forward_align.readlines()
# 	# print(lines)

# 	# for line in forward_align:
# 	# 	# for element in range(0, len(line)):
# 	# 	print(line)

# 	# 	forward_align.close()

# 	# print("ALIGN DICT HOLAAAAA")

# 	print(align_dict)
# 	align_file.write(str(align_dict))
# 	align_file.write("\n")
...
# def classification(mapping, instance1, instance2, attributes1, attributes2, relation1, relation2):

#     instance1 = [(normalize(x),normalize(y),normalize(z)) for x,y,z in instance1]
#     instance2 = [(normalize(x),normalize(y),normalize(z)) for x,y,z in instance2]
#     attributes1 = [(normalize(x),normalize(y),normalize(z)) for x,y,z in attributes1]
#     attributes2 = [(normalize(x),normalize(y),normalize(z)) for x,y,z in attributes2]
#     relation1 = [(normalize(x),normalize(y),normalize(z)) for x,y,z in relation1]
#     relation2 = [(normalize(x),normalize(y),normalize(z)) for x,y,z in relation2]

#     # for a,b,c in instance1:
#     #     normalize
#     num_divergences = 0
#     divergences = []

#     connects1 = attributes1 + relation1
#     connects1_unchanging = attributes1 + relation1
#     connects2 = attributes2 + relation2
#     connects2_unchanging = attributes2 + relation2

#     # Different Focus [X]
#     # CHECK THIS !!!!!!!!!
#     if attributes1[0][2] != attributes2[0][2]: 
#         num_divergences += 1
#         appendage = "diff_root"
#         divergences.append(appendage)
#         return num_divergences, divergences

#     connects1.remove(attributes1[0])
#     connects2.remove(attributes2[0])

#     # Added/Omitted Non-Core Role
#     # if len(attributes1) > 1 and len(attributes2) > 1:
#     #     for item1 in attributes1[1:]:
#     #         for j, item2 in attributes2[1:]:
#     #             if item1[0] == item2[0] and instance1[i][1] == instance2[m][1]

#     for i, m in enumerate(mapping):
#         if m == -1:
#             # Added/Omitted Arg
#             if isarg(instance1[i],attributes1,relation1):
#                 num_divergences += 1
#                 appendage = "omitarg " + instance1[i][2]
#                 divergences.append(appendage)

#             # Added / Omitted Non-Core Role
#             else:
#                 num_divergences += 1
#                 appendage = "omitnoncore " + instance1[i][2]
#                 divergences.append(appendage)

#             for item in connects1:
#             	if find_type(item,attributes1,relation1) == 'relation':
#             		if item[1] == instance1[i][1] or item[2] == instance1[i][1]:
#             			connects1.remove(item)
#             	if find_type(item,attributes1,relation1) == 'attribute':
#             		if item[1] == instance1[i][1]:
#             			connects1.remove(item)

#         else:

#             # Different Arg Chosen
#             if isarg(instance1[i],connects1,relation1) and isarg(instance2[m],connects2,relation2) and instance1[i][0] == instance2[m][0] and aligns_with(mapping,instance1[i][1],instance2[m][1]) and instance1[i][2] != instance2[m][2]:
#                 num_divergences += 1
#                 appendage = "diffarg " + instance1[i][2] + " " + instance2[m][2]
#                 divergences.append(appendage)

#             # Different Non-Core Role Chosen
#             elif not isarg(instance1[i],connects1,relation1) and not isarg(instance2[m],connects2,relation2) and instance1[i][0] == instance2[m][0] and aligns_with(mapping,instance1[i][1],instance2[m][1]) and instance1[i][2] != instance2[m][2]:
#                 num_divergences += 1
#                 appendage = "diffnoncore " + instance1[i][2] + " " + instance2[m][2]
#                 divergences.append(appendage)

#             elif not isarg(instance1[i],connects1,relation1) and isarg(instance2[m],connects2,relation2) and instance1[i][0] == instance2[m][0] and aligns_with(mapping,instance1[i][1],instance2[m][1]) and instance1[i][2] != instance2[m][2]:
#                 num_divergences += 1
#                 appendage = "switch " + instance1[i][2] + " " + instance2[m][2]
#                 divergences.append(appendage)
...
#                 for item in connects2:
#                 	if find_type(item,attributes2,relation2) == 'relation':
#                 		if item[1] == instance2[m][1] or item[2] == instance2[m][1]:
#                 			connects2.remove(item)
#                 	if find_type(item,attributes2,relation2) == 'attribute':
#                 		if item[1] == instance2[m][1]:
#                 			connects2.remove(item)

#             elif isarg(instance1[i],connects1,relation1) and not isarg(instance2[m],connects2,relation2) and instance1[i][0] == instance2[m][0] and aligns_with(mapping,instance1[i][1],instance2[m][1]) and instance1[i][2] != instance2[m][2]:
#                 num_divergences += 1
#                 appendage = "switch " + instance1[i][2] + " " + instance2[m][2]
#                 divergences.append(appendage)
...
#     for item1 in connects1:
#         for item2 in connects2:
#         	if find_type(item1,attributes1,relation1) == 'relation':
#         		if find_type(item2,attributes2,relation2) == 'relation':
#         			if item1[0] == item2[0] and aligns_with(mapping,item1[1],item2[1]) and aligns_with(mapping,item1[2],item2[2]):
#         				remove_connects1.append(item1)
#         				remove_connects2.append(item2)
#         		# elif find_type(item2) == 'attribute':
#         		# 	# something
#         	elif find_type(item1,attributes1,relation1) == 'attribute':
#         		# if find_type(item2) == 'relation':
#         		# 	# something
#         		if find_type(item2,attributes2,relation2) == 'attribute':
#         			if item1[0] == item2[0] and aligns_with(mapping,item1[1],item2[1]) and item1[2]==item2[2]:
#         				remove_connects1.append(item1)
#         				remove_connects2.append(item2)


#     connects1 = [x for x in connects1 if x not in remove_connects1]
#     connects2 = [x for x in connects2 if x not in remove_connects2]
...
#     for item in connects1:
#         if isarg(item,connects1,relation1):
#             num_divergences += 1
#             appendage = "omitarg " + item[2]
#             divergences.append(appendage)
#         else:
#             num_divergences += 1
#             appendage = "omitnoncore " + item[2]
#             divergences.append(appendage)  

#     for item in connects2:
#         if isarg(item,connects2,relation2):
#             num_divergences += 1
#             appendage = "omitarg " + item[2]
#             divergences.append(appendage)
#         else:
