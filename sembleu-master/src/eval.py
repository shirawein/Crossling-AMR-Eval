...
def truncate(text, end_index=5):
	if len(text) >= end_index:
		if text.startswith('"') and text.endswith('"'):
			new_text = text[1:end_index]
			if new_text.endswith('"'):
				new_text = new_text[1:(end_index-1)]
			return new_text
		else:
...
def translate_and_truncate_amr(path):
	...
	#data = []
	english_tokens = []
	english_tokens.append("temporal-quantity")
	english_tokens.append("name")
	english_tokens.append("age-01")
	english_tokens.append("I")
	english_tokens.append("II")
	english_tokens.append("III")
		...
		for line in read_amr(path):
			...
			for item in line.split():
				...
				if item == "/":
					...
					continue
				if item == "我":
					file_save_path.write("i ")
					continue
				if re.match(r"[cC]\d{1,2}", item):
					file_save_path.write(item)
					file_save_path.write(' ')
					continue
				# if ("丝毫") in item:
				# 	file_save_path.write("smallest ")
				# 	continue
				if item.startswith(':'):
					file_save_path.write(item)
					file_save_path.write(' ')
				...
				# 	continue
					...
					if item == "我":
						...
						f.close()
						translator.load_sentences('line.txt')
						out = translator.translate(source_language="zh")
						out_text = str(out[0]).lower()
						print("TRANSLATION")
						print("item: ", item)
